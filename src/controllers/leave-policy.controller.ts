...
export class LeavePolicyController {
  ...
  static async create(req: Request, res: Response): Promise<void> {
    try {
      const result = await LeavePolicyService.create(req.body);
      const leavePolicyDto = toDto(LeavePolicyResponseDto, result);
      const response = ResponseUtil.success(INFO_MESSAGES.LEAVE_POLICY.CREATED_SUCCESSFULLY, leavePolicyDto);
